 ...
 *
 * ===============================================
 * Name: mysql.db.class.php
 * ===============================================
 * Beschreibung:
 * Class fuer einfache mySQL Verwendung
 *
 * ===============================================
 *
 * Author: Niopthen
...
class eylib_mysql extends eylib
{
		...
		switch ($this->DB_connection_data['DB_CONNECTION_TYP'])
		{
			case 'connect':
				$this->DB_Handle = @mysql_connect(
					$this->DB_connection_data['DB_SERVER_IP'] . ":" . $this->DB_connection_data['DB_PORT'], $this->DB_connection_data['DB_USER_NAME'], $this->DB_connection_data['DB_USER_PASSWORD']
				);
	...
	public function DB_Select($query)
	{
		...
		if (mysql_error() == '')
		{
			...
			{
				$this->getFirstRow = $result[0];
				$this->getLastRow = $result["$this->num_rows - 1"];
			}
	...
	{
		if ($this->DB_connection_data['db_contyp'] != 'pconnect')
		{
			mysql_close();
		}
